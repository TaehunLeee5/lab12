...
public class FamilyTree
{
    ...
    private static class TreeNode
    {
        ...
        ArrayList<TreeNode> collectAncestorsToList()
        {
            ...
            TreeNode focusNode = this.parent;
            
            while(focusNode !=null) {
            	ancestors.add(focusNode);
	...
	private void addLine(String line) throws TreeException
	{
		...
		// Find par3ent node. If root is null then the tree is empty and the
	...
	//
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws Exception
	{
		// Get nodes for input names.
		TreeNode node1 = root.getNodeWithName(name1);		// node whose name is name1
		if (node1 == null)
			throw new Exception("No Ancestor"); //??? Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new Exception("No Ancestor"); //??? Throw TreeException with a useful message
